...
public class Rocket {
	...
	//We need a method that returns as String the boosters maximum power and the actual power
	public String getMaximumPowerAndActualPowerBooster() {
		String maximumPowerBooster = "";
		for (Booster booster: this.boosters) {
			maximumPowerBooster += "potencia máxima: "+ booster.getmaximumPower() + ", ";
			maximumPowerBooster += "potencia actual: " + booster.getActualPower() +" \n";
		}
		return maximumPowerBooster;
	}
	
	//This prints the whole description of the rocket
	public String printAll() {
		String discription = "id: " + this.id + " propulsores: " + this.getMaximumPowerAndActualPowerBooster()+"n";
		return discription;
	}
	...
	/*
	 * I created this method in order to accelerate all boosters to a certain value, but finally I didn't need it	
	//Here we want an accelerator method
	public void accelerateBoosters() {
		System.out.println(this.printAll());//We give the original conditions
		for (Booster booster: this.boosters) {
			booster.accelerate(40);
		}
		System.out.println(this.printAll());//We give the the final conditions
	}
	*/

	/*
	 * I created this method in order to decelerate all boosters to a certain value, but finally I didn't need it
	public void decelerateBooster() {
		System.out.println(this.printAll());//We give the original conditions
		for (Booster booster: this.boosters) {
			booster.accelerate(10);
		}
		System.out.println(this.printAll());//We give the the final conditions
	}
	*/
